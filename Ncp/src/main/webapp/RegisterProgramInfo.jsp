			...
			<section>
				<div>
					...
					<div id="tableArea" class="tableArea show">
							...
							<table>
								<tbody>
									<tr>
										<th>프로그램 코드</th>
										<td>
											<input id="code" type="text" name="code" value="${program.code }" />
										</td>
										<th>프로그램 명</th>
										<td>
											<input id="program_name" type="text" name="program_name" value="${program.program_name }" />
										</td>
									</tr>
									<tr>
										<th>운영 유형</th>
										<td>
											<select id="operating_type" name="operating_type">
												<c:forEach items="${operatingTypeList}" var="operatingType" varStatus="status">
													<c:choose>
														<c:when test="${operatingType.code eq program.operating_type }">
										...
										<th>이수시간</th>
										<td>
											<input id="completation_time" type="number" name="completation_time" value="${program.completation_time }" />
											시간
										</td>
									</tr>
									<tr>
										...
										<td>
											<select id="category_large" name="category_large">
												<c:forEach items="${largeCategoryList}" var="largeCategory" varStatus="status">
													<c:choose>
														<c:when test="${largeCategory.code eq program.category_large}">
												...
												<c:forEach items="${middleCategoryList}" var="middleCategory" varStatus="status">
													<c:choose>
														<c:when test="${middleCategory.code eq program.category_middle}">
							...
							<table>
								<tbody>
									...
									<tr>
										<th>반영률</th>
										<td>
											<input id="TALENT_capability_01" type="number" name="TALENT_capability_01" value="${TALENT_capability[0] }" />
										</td>
										<td>
											<input id="TALENT_capability_02" type="number" name="TALENT_capability_02" value="${TALENT_capability[1] }" />
										</td>
										<td>
											<input id="TALENT_capability_03" type="number" name="TALENT_capability_03" value="${TALENT_capability[2] }" />
										</td>
										<td>
											<input id="TALENT_capability_04" type="number" name="TALENT_capability_04" value="${TALENT_capability[3] }" />
										</td>
										<td>
											<input id="TALENT_capability_05" type="number" name="TALENT_capability_05" value="${TALENT_capability[4] }" />
										</td>
										<td>
											<input id="TALENT_capability_06" type="number" name="TALENT_capability_06" value="${TALENT_capability[5] }" />
										</td>
									</tr>
								</tbody>
							</table>
							...
							<table>
								<tbody>
									<tr>
										<th>프로그램 목표</th>
										<td>
											<textarea id="program_gaol" name="program_goal" placeholder="입력하세요"><c:out value="${program.program_goal }"></c:out></textarea>
										</td>
									</tr>
									<tr>
										<th>프로그램 세부내용</th>
										<td>
											<textarea id="program_detail" name="program_detail" placeholder="입력하세요"><c:out value="${program.program_detail }"></c:out></textarea>
										</td>
									</tr>
								</tbody>
							</table>
						...
						<div class="btnArea">
							<button id="btnRemove" class="btnRemove" onclick="deleteProgramInformation();">삭제</button>
							<button id="btnRegister" class="btnRegister" onclick="updateProgramInformation();">수정</button>
						</div>
					</div>
